<template>
  <div class="login-container">
    ...
    <div class="login-header">
      <a :class="{ active: loginway == 'login' }" @click="changeLogin('login')">
        &nbsp;&nbsp;登录&nbsp;&nbsp;▪
      </a>
      <a :class="{ active: loginway == 'register' }" @click="changeLogin('register')">
        &nbsp;&nbsp;注册&nbsp;&nbsp;
      </a>
    </div>
    <!-- 登录界面开始 -->
    <div class="login-input" v-if="loginway === 'login'">
      <form class="form_login" accept-charset="UTF-8" method="post">
        <div class="username">
          ...
          <input class="span2" type="text" name="register_name" placeholder="用户名">
        </div>
        <div class="password">
          <span class="span1"><i class="fa fa-unlock-alt"></i></span>
          <input class="span2" type="password" name="register_name" placeholder="密码">
        </div>
        <button class="login-btn btn" type="submit"><span>登录</span></button>
        <div class="login-control" style="color: #555555;font-size: 12px">
          <span style="float: left" class="checkbox" :class="{ checked: checked === true }" @click="checked = !checked">
            <input type="checkbox" style="position: absolute;opacity: 0;" checked="checked"> <!-- 勾选框 -->
            <ins class="check"></ins> <!--H5<ins> 标签定义文档的其余部分之外的插入文本。-->
          </span>
    ...
    <div class="login-input" v-if="loginway === 'register'">
      <form class="form_register" accept-charset="UTF-8" method="post">
        <div class="email">
          ...
          <input class="span2" type="text" name="email_adress" placeholder="输入一个邮箱地址">
        </div>
        <div class="username">
          <span class="span1"><i class="fa fa-user"></i></span>
          <input class="span2" type="text" name="sign_in[name]" placeholder="用户名/手机号码/电子邮件">
        </div>
        <div class="password">
          <span class="span1"><i class="fa fa-unlock-alt"></i></span>
          <input class="span2" type="password" name="sign_in[password]" placeholder="密码">
        </div>
        <button class="register-btn btn" type="submit"><span>注册</span></button>
        <p class="register_text">点击 “注册” 或通过以下第三方软件授权注册账户，即表示您同意并愿意遵守该网站的 <a href="#">用户协议</a> 和 <a href="#">隐私政策</a> 。</p>
      </form>
    </div>
    <!-- 注册界面结束 -->
    <div class="login-way">
      ...
      <ul>
        ...
        <li class="googleicon"><i class="fa fa-google"></i></li>
        <li class="facebookicon"><i class="fa fa-facebook"></i></li>
